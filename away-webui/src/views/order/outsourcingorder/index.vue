        ...
        <el-col :span="21">
          <div style="overflow-x: auto;scrollbar-width: none; white-space: nowrap;">
            <el-form-item label="委外类型" prop="outsourcingType">
              <el-select v-model="queryParams.outsourcingType" placeholder="请选择委外类型" clearable>
                <el-option v-for="dict in dict.type.aw_order_outsourcingorder_type" :key="dict.value" :label="dict.label"
                  :value="dict.value" />
              </el-select>
            </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['order:outsourcingorder:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['order:outsourcingorder:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['order:outsourcingorder:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
          v-hasPermi="['order:outsourcingorder:export']">导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="outsourcingorderList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="外协单编号" align="center" prop="outsourcingOrderID" />
      <el-table-column label="任务单编号" align="center" prop="productionTasksFormID" />
      ...
      <el-table-column label="委外类型" align="center" prop="outsourcingType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.aw_order_outsourcingorder_type" :value="scope.row.outsourcingType" />
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-view" @click="handleView(scope.row)"
            v-hasPermi="['order:outsourcingorder:edit']">查看</el-button>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="外协单编号" prop="outsourcingOrderID">
          <el-input v-model="form.outsourcingOrderID" placeholder="请输入外协单编号" />
        </el-form-item>
        <el-form-item label="任务单编号" prop="productionTasksFormID">
          <el-input v-model="form.productionTasksFormID" placeholder="请输入任务单编号" />
        </el-form-item>
        <el-form-item label="任务编号" prop="productionTasksID">
          <el-input v-model="form.productionTasksID" placeholder="请输入任务编号" />
        </el-form-item>
        <el-form-item label="委外类型" prop="outsourcingType">
          <el-select v-model="form.outsourcingType" placeholder="请选择委外类型">
            <el-option v-for="dict in dict.type.aw_order_outsourcingorder_type" :key="dict.value" :label="dict.label"
              :value="dict.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item v-if="form.outsourcingType == 1" label="工序编号" prop="processingprocessID">
...
<script>
import { listOutsourcingorder, getOutsourcingorder, delOutsourcingorder, addOutsourcingorder, updateOutsourcingorder } from "@/api/order/outsourcingorder";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除外协编号为"' + outsourcingOrderIDs + '"的数据项？').then(function () {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('order/outsourcingorder/export', {
        ...this.queryParams
