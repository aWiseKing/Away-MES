<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="shippinginspectionList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="检测结果" align="center" prop="testResult">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.aw_quality_shippinginspection_status" :value="scope.row.testResult"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="出货数量" prop="shipmentQuantity">
          <el-input v-model="form.shipmentQuantity" placeholder="请输入出货数量" />
        </el-form-item>
...
<script>
import { listShippinginspection, getShippinginspection, delShippinginspection, addShippinginspection, updateShippinginspection } from "@/api/quality/shippinginspection";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除成品出货检验编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('quality/shippinginspection/export', {
        ...this.queryParams
