        ...
        <el-button
         ...
         <el-button
          ...
          <el-button
            ...
            v-if="scope.row.status == '1'"
            v-hasPermi="['storage:CustomerMaterialsInventory:reducenumber']"
            >完成出库</el-button
          >
          <el-button
            size="mini"
...
import {
  ...
  delDetailCustomerMaterialOutbound,
  addDetailCustomerMaterialOutbound,
  updateDetailCustomerMaterialOutbound,
...
export default {
  ...
  data() {
    return {
      ...
      total: 0,
      // 客供材料出库单表格数据
      CustomerMaterialsOutboundFormList: [],
      // 弹出层标题
      title: "",
      // 是否显示弹出层
      open: false,
  ...
  methods: {
    ...
    handleView(row) {
      ...
      let deliveryNoteID = row.deliveryNoteID;
      this.$router.push({
        path: "DetailCustomerMaterialOutbound",
        query: { id: deliveryNoteID, status: row.status },
      });
    },
    /** 新增按钮操作 */
    handleAdd() {
