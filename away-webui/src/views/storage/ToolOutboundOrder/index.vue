<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="ToolOutboundOrderList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="刀具出库编号" prop="deliveryNoteID">
          <el-input v-model="form.deliveryNoteID" placeholder="请输入刀具出库编号" />
        </el-form-item>
        <el-form-item label="出库日期" prop="deliveryDate">
          <el-date-picker clearable
...
<script>
import { listToolOutboundOrder, getToolOutboundOrder, delToolOutboundOrder, addToolOutboundOrder, updateToolOutboundOrder } from "@/api/storage/ToolOutboundOrder";
...
export default {
  ...
  methods: {
   ...
   async handleFinish(row) {
      ...
      listDetailsOfToolDelivery({pageSize:total,deliveryNoteID:deliveryNoteID}).then((response)=>{
        ...
        if(this.DetailsOfToolDeliveryList.length > 0){
          ...
          for(let num in this.DetailsOfToolDeliveryList){
            let toolInformationID = this.DetailsOfToolDeliveryList[num].toolInformationID;
    ...
    handleView(row) {
      this.view_open = true;
      this.$router.push({path:"DetailsOfToolDelivery",query:{id:row.deliveryNoteID,status:row.status}})
    },
    /** 新增按钮操作 */
    handleAdd() {
      this.reset();
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除刀具出库单编号为"' + deliveryNoteIDs + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('storage/ToolOutboundOrder/export', {
        ...this.queryParams
