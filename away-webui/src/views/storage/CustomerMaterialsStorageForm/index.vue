<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['storage:CustomerMaterialsStorageForm:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['storage:CustomerMaterialsStorageForm:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['storage:CustomerMaterialsStorageForm:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
    ...
    <el-table
      v-loading="loading"
      :data="CustomerMaterialsStorageFormList"
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-hasPermi="['storage:CustomerMaterialsInventory:addnumber']"
            >完成入库</el-button
          >
...
<script>
import {
  listCustomerMaterialsStorageForm,
  getCustomerMaterialsStorageForm,
  delCustomerMaterialsStorageForm,
  addCustomerMaterialsStorageForm,
  updateCustomerMaterialsStorageForm,
} from "@/api/storage/CustomerMaterialsStorageForm";

import { listDetailCustomerMaterialsStorage } from "@/api/storage/DetailCustomerMaterialsStorage";

import { addByNumber } from "@/api/storage/CustomerMaterialsInventory";
export default {
  name: "CustomerMaterialsStorageForm",
  ...
  data() {
    return {
      ...
      total: 0,
      // 客供入库单表格数据
      CustomerMaterialsStorageFormList: [],
      // 弹出层标题
      title: "",
      // 是否显示弹出层
      open: false,
      ...
      DetailCustomerMaterialsStorage: {},
    };
  },
  created() {
    this.getList();
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCustomerMaterialsStorageForm(this.queryParams).then((response) => {
        this.CustomerMaterialsStorageFormList = response.rows;
    ...
    handleView(row) {
      ...
      this.$router.push({
        path: "DetailCustomerMaterialsStorage",
    ...
    setStatus(row, status) {
      ...
      getCustomerMaterialsStorageForm(warehouseEntryID).then((response) => {
        this.form = response.data;
        this.form.status = status;
        updateCustomerMaterialsStorageForm(this.form).then((response) => {
    ...
    handleWarehousing(row) {
      ...
      listDetailCustomerMaterialsStorage({
        ...
        this.DetailCustomerMaterialsStorage = response.rows;
        if (this.DetailCustomerMaterialsStorage.length > 0) {
          this.setStatus(row, "2");
        } else {
    ...
    async handleFinish(row) {
      let total = (await listDetailCustomerMaterialsStorage())["total"];
      let warehouseEntryID = row.warehouseEntryID;
      this.loading = true;
      listDetailCustomerMaterialsStorage({
        warehouseEntryID: warehouseEntryID,
        ...
        console.log(this.DetailCustomerMaterialsStorageList);
        if (this.DetailCustomerMaterialsStorageList.length > 0) {
          let lmlist = [];
          for (let num in this.DetailCustomerMaterialsStorageList) {
            let materialID =
              this.DetailCustomerMaterialsStorageList[num].materialID;
            let value =
              this.DetailCustomerMaterialsStorageList[num].receiptQuantity;
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (!this.isadd) {
            ...
            addCustomerMaterialsStorageForm(this.form).then((response) => {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm(
          '是否确认删除客供入库单编号为"' + warehouseEntryIDs + '"的数据项？'
        )
        .then(function () {
          return delCustomerMaterialsStorageForm(warehouseEntryIDs);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      this.download(
        "storage/CustomerMaterialsStorageForm/export",
        {
          ...this.queryParams,
        },
        `CustomerMaterialsStorageForm_${new Date().getTime()}.xlsx`
      );
    },
  },
};
