    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['storage:customersuppliedmaterials:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['storage:customersuppliedmaterials:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['storage:customersuppliedmaterials:export']"
        >导出</el-button>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCustomersuppliedmaterials(this.queryParams).then(response => {
        this.customersuppliedmaterialsList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (!this.isadd) {
            ...
            addCustomersuppliedmaterials(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除客供材料实时库存编号为"' + ids + '"的数据项？').then(function() {
        return delCustomersuppliedmaterials(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('storage/customersuppliedmaterials/export', {
        ...this.queryParams
      }, `customersuppliedmaterials_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
