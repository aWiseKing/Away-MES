<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="orderhoursdetailedList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />

      <el-table-column label="订单编号" align="center" prop="id" />
      <el-table-column label="销售单创建日期" align="center" prop="createTime" width="180">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="需求数量#要求大于0#" prop="number">
          <el-input v-model="form.number" placeholder="请输入需求数量#要求大于0#" />
        </el-form-item>
        <el-form-item label="实体姓名" prop="name">
...
<script>
import { listOrderhoursdetailed, getOrderhoursdetailed, delOrderhoursdetailed, addOrderhoursdetailed, updateOrderhoursdetailed } from "@/api/produce/orderhoursdetailed";
import {
  listProduct,
  getProduct,
  delProduct,
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        id: [
          { required: true, message: "订单id#日期+编号#不能为空", trigger: "blur" }
        ],
        createTime: [
          { required: true, message: "销售单创建日期不能为空", trigger: "blur" }
        ],
        number: [
          { required: true, message: "需求数量", trigger: "blur" }
        ],
        name: [
  ...
  methods: {
    ...
    handleView(row) {
       this.view_open = true;
      this.$router.push({path:"productmanhour",query:{id:row.id,number:row.number}})
    },
//查看产品
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除工时统计编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('produce/orderhoursdetailed/export', {
        ...this.queryParams
