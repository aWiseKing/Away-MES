    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['produce:productoutbound:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['produce:productoutbound:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['produce:productoutbound:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="productoutboundList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="出货检验编号" align="center" prop="shippingInspectionID" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="出货检验编号" prop="shippingInspectionID">
          <el-input v-model="form.shippingInspectionID" placeholder="请输入出货检验编号" />
        </el-form-item>
        <el-form-item label="出库数量" prop="receiptQuantity">
          <el-input v-model="form.receiptQuantity" placeholder="请输入出库数量" />
        </el-form-item>
        <el-form-item label="客户编号" prop="contractID">
          <el-input v-model="form.contractID" placeholder="请输入客户编号" />
        </el-form-item>
...
<script>
import { listProductoutbound, getProductoutbound, delProductoutbound, addProductoutbound, updateProductoutbound } from "@/api/produce/productoutbound";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listProductoutbound(this.queryParams).then(response => {
        this.productoutboundList = response.rows;
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除产品出库编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('produce/productoutbound/export', {
        ...this.queryParams
      }, `productoutbound_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
