    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['produce:processrequirementmateriallist:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['produce:processrequirementmateriallist:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['produce:processrequirementmateriallist:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['produce:processrequirementmateriallist:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listProcessrequirementmateriallist, getProcessrequirementmateriallist, delProcessrequirementmateriallist, addProcessrequirementmateriallist, updateProcessrequirementmateriallist } from "@/api/produce/processrequirementmateriallist";
export default {
  ...
  data() {
    return {
      ...
      rules: {
        materialID: [
          { required: true, message: "材料基础信息编号不能为空", trigger: "change" }
        ],
        processingTechnologyID: [
          { required: true, message: "工艺编号不能为空", trigger: "change" }
        ],
        cuttingSize: [
  ...
  methods: {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('produce/processrequirementmateriallist/export', {
        ...this.queryParams
